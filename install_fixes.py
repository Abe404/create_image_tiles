...
def fix_app():
    ...
    # ./target/create_image_tiles.app
    is_mac = os.path.isdir('./target/create_image_tiles.app')
    is_linux = os.path.exists('./target/create_image_tiles/create_image_tiles')
    
    # or the following folder on windows
    is_windows = os.path.exists('target\create_image_tiles\create_image_tiles.exe')
   
    print('is_windows', is_windows)
    print('is_mac', is_mac)
    print('is_linux', is_linux)

    # If you try to run on the command line
    ...
    if is_mac:
        ...
        build_dir = './target/create_image_tiles.app/Contents/MacOS/'
    elif is_windows:
        site_packages_dir = os.path.join(env_dir, 'Lib', 'site-packages')
        build_dir = './target/create_image_tiles'
    elif is_linux:
